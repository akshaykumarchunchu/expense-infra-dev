...
# connect to database server using null resource and remote exec ##
resource "null_resource" "backend" {
    triggers = {
      instance_id = module.backend.id       # this will be triggered everytime instance is created
    }

# Connection is between backend private_ip to remote server ##
    connection {
        type     = "ssh"
        user     = "ec2-user"
        password = "DevOps321"
...
## Take the ami ID and delete the instance ##
